...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if ($firstUserId !== null) {
            DB::table('alternatives')->whereNull('user_id')->update(['user_id' => $firstUserId]);
        }

        // 3. Remove any rows that still cannot be assigned an owner
        //    (their alternative_values cascade automatically).
        DB::table('alternatives')->whereNull('user_id')->delete();

        // 4. Enforce NOT NULL and switch the unique constraint to be per-user.
        Schema::table('alternatives', function (Blueprint $table) {
            $table->foreignId('user_id')->nullable(false)->change();
            $table->dropUnique('alternatives_code_unique');
